/**
 * @fileoverview NCBL 加密工具 —— 复制自 @netease-report-listen-song
...
const rsaWrap = (keyA: Buffer): Buffer => {
...
const getCompress = (): Compressor => {
  const zstd = (zlib as any).zstdCompressSync;
  if (typeof zstd === 'function') {
    return { compress: (buf) => zstd(buf), name: 'zstd' };
  }
  return { compress: (buf) => zlib.gzipSync(buf), name: 'gzip' };
};

/** NCBL 加密选项 */
interface EncryptNCBLOpts {
  maxFrame?: number;
  keyA?: Buffer;
  uuid?: Buffer;
  baseSeq?: number;
...
const buildRecord = ({ time, action, data }: LogRecord): string => {
  const json = typeof data === 'string' ? data : JSON.stringify(data);
  return [time, action, json].join(FIELD_SEP);
};

const buildRecords = (records: LogRecord[]): string =>
  records.map(buildRecord).join('');

// ---- PLV / PLD 构建器 (桌面客户端格式) ----
const buildPlv = (ctx: any, song: any, source: any) => {
  const now = Date.now();
...
const buildPld = (ctx: any, song: any, source: any, played: number) => {
  const now = Date.now();
...
// ---- Cookie → 设备/认证上下文 转换 ----
const extractContext = (cookieObj: any) => {
  return {
    app: {
      id: cookieObj.appid || '',
      urs: '',
...
const parseCookie = (
  cookie: string | Record<string, any> | null | undefined,
): Record<string, any> => {
...
const buildMultipart = (
  payload: Buffer,
): { boundary: string; fileName: string; multipartBody: Buffer } => {
...
const buildCookieStr = (ctx: any): string => {
  const parts = [
...
const buildMetaJson = (ctx: any): string =>
  JSON.stringify({
    'JSESSIONID-WYYY': ctx.auth.sessionId,
    MUSIC_U: ctx.auth.token,
    NMTID: ctx.device.ti,
