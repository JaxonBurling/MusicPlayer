/**
 * @fileoverview 客户端签名生成器
 * @module util/client-sign
 */
...
class AdvancedClientSignGenerator {
   ...
   * 获取本机MAC地址
   */
  static getRealMacAddress(): string | null {
    try {
      const interfaces = os.networkInterfaces();
      for (const interfaceName in interfaces) {
        const interfaceItem = interfaces[interfaceName];
        if (!interfaceItem) continue;
      ...
      console.warn('获取MAC地址失败:', message);
      return null;
    }
  }
   ...
   */
  static generateRandomClientSign(secretKey = ''): string {
    // 获取MAC地址（优先真实，否则随机）
   ...
   * 批量生成多个随机签名
   */
  static generateMultipleRandomSigns(count: number, secretKey = ''): string[] {
    const signs: string[] = [];
   ...
   * 使用指定参数生成签名
   */
  static generateWithCustomDeviceId(
    macAddress: string,
    deviceId: string,
    secretKey = '',
  ): string {
   ...
   * 验证签名格式是否正确
   */
  static validateClientSign(clientSign: string): boolean {
    try {
...
export default AdvancedClientSignGenerator;
