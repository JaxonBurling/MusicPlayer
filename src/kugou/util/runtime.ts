 ...
 */
let cachedProxy: AxiosProxyConfig | null | undefined;
 ...
 * - key 和 value 均不能为空
 *
 * @param args 参数数组，默认使用 process.argv.slice(2)
 * @returns 解析后的键值对对象
 *
 * @example
...
function parseCliArgs(args?: string[]): Record<string, string> {
  ...
  return source.reduce<Record<string, string>>((acc, rawArg) => {
 ...
 * @param args 参数数组，默认使用 process.argv.slice(2)
 */
function applyCliOverrides(args?: string[]): void {
 ...
 */
function resolveProxy(): AxiosProxyConfig | null {
  ...
  try {
    ...
    console.warn(`[proxy] Failed to parse proxy address "${rawProxy}": ${message}`);
    cachedProxy = null;
  }
...
export { applyCliOverrides, parseCliArgs, resolveProxy };
