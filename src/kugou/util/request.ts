...
import { resolveProxy } from './runtime';
import { generateSimulate } from './generate_simulate';
import type { UseAxiosOptions, UseAxiosResponse } from './types';
 ...
 * @returns 统一格式的响应对象
 */
const createRequest = (options: UseAxiosOptions): Promise<UseAxiosResponse> => {
  return new Promise(async (resolve, reject) => {
    ...
    const headers: Record<string, any> = { dfid, clienttime, mid, 'kg-rc': '1', 'kg-thash': '5d816a0', 'kg-rec': 1, 'kg-rf': 'B9EDA08A64250DEFFBCADDEE00F8F25F' };
    ...
    const requestOptions: AxiosRequestConfig = {
      ...
      responseType: options.responseType as AxiosRequestConfig['responseType'], // 响应类型（如 'arraybuffer'）
    };
    ...
    try {
      ...
      answer.cookie = ((response.headers['set-cookie'] || []) as string[]).map((x) => parseCookieString(x));
