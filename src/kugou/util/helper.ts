...
import { Hex, MD5Algo, Utf8 } from 'crypto-es';
import { cryptoMd5, wordArrayFromBuffer } from './crypto';
 ...
 * @returns MD5 签名字符串（32位小写hex）
 */
const signatureWebParams = (params: Record<string, any>): string => {
 ...
 *
 * @param params 请求参数键值对
 * @param data 可选的请求体数据（如 POST body）
 * @returns MD5 签名字符串
 */
const signatureAndroidParams = (params: Record<string, any>, data?: string | Uint8Array): string => {
  ...
  if (data instanceof Uint8Array) {
    ...
    hasher.update(Utf8.parse(str));
    hasher.update(Utf8.parse(paramsString));
 ...
 * @returns MD5 签名字符串
 */
const signatureRegisterParams = (params: Record<string, any>): string => {
  const paramsString = Object.keys(params)
 ...
 * @returns MD5 签名字符串
 */
const signParams = (params: Record<string, any>, data?: string | Uint8Array): string => {
 ...
 * @param appid 应用 ID，默认使用配置文件中的值
 * @returns MD5 签名字符串
 */
const signKey = (hash: string, mid: string, userid?: string | number, appid?: string | number): string => {
  const isLite = process.env.platform === 'lite';
  // 标准版和概念版使用不同的盐值
 ...
 * @returns MD5 签名字符串
 */
const signCloudKey = (hash: string, pid: string | number): string => {
 ...
 *
 * @param data 签名数据（通常为请求哈希或时间戳）
 * @param appid 应用 ID，默认使用配置文件中的值
 * @param clientver 客户端版本号，默认使用配置文件中的值
 * @returns MD5 签名字符串
 */
const signParamsKey = (data: string | number, appid?: string | number, clientver?: string | number): string => {
  const isLite = process.env.platform === 'lite';
  // 标准版和概念版使用不同的盐值
  const str = isLite ? 'LnT6xpN3khm36zse0QzvmgTZ3waWdRSA' : 'OIlwieks28dk2k092lksi2UIkp';
