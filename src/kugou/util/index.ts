 ...
 * 3. 统一导出供 API 模块使用
 *
 * API 模块（module/ 目录下的文件）通过 `import { ... } from '../util'` 引用本文件，
 * 即可获得所有需要的工具函数和配置常量。
 *
 * @module util/index
 */
...
// ========== 工具函数 ==========
import { randomString, decodeLyrics, parseCookieString, cookieToJson, randomNumber, calculateMid } from './util';
...
export {
  // --- 配置常量 ---
  apiver,                       // API 版本号
  useAppid as appid,            // 应用 ID（根据平台自动选择）
  wx_appid,                     // 微信小程序应用 ID
  wx_lite_appid,                // 微信概念版小程序应用 ID
  wx_secret,                    // 微信小程序密钥
  wx_lite_secret,               // 微信概念版小程序密钥
  srcappid,                     // 来源应用 ID
  useClientver as clientver,    // 客户端版本号（根据平台自动选择）
  isLite,                       // 是否为概念版
