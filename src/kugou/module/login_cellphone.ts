...
// 手机登录
import { cryptoAesDecrypt, cryptoAesEncrypt, cryptoRSAEncrypt, signParamsKey, isLite, randomString } from '../util';
        ...
        if (body?.status && body?.status === 1) {
          if (body?.data?.secu_params) {
            ...
            if (typeof getToken === 'object') {
              ...
              Object.keys(getToken).forEach((key: any) => res.cookie.push(`${key}=${getToken[key]}`));
            } else {
