...
import { wx_appid, wx_secret, cryptoAesDecrypt, cryptoAesEncrypt, cryptoRSAEncrypt, wx_lite_appid, wx_lite_secret, isLite } from '../util';
      ...
      if (assetsTokenResp.data?.access_token && assetsTokenResp.data?.openid) {
        ...
        if (response.body?.status === 1) {
          ...
          if (typeof getToken === 'object') {
            ...
            Object.keys(getToken).forEach((key: any) => response.cookie.push(`${key}=${getToken[key]}`));
          } else {
