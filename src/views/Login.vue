...
async function startQrLogin() {
  ...
  try {
    const key = await getQrKey(store.source)
    if (!key) { qrStatus.value = '获取二维码失败'; return }

    const img = await createQr(store.source, key)
    qrImage.value = img
  ...
  <div class="login-page">
    <div class="login-card">
      ...
      <div v-if="tab === 'qr'" class="qr-section">
        ...
        <el-button text type="primary" @click="startQrLogin" :loading="!qrImage && qrStatus !== '网络错误，请重试'">
          重新生成二维码
        </el-button>
      </div>
